...
class tree //之前写这个第一个括号总会报错,只能重新复制一个
{
  public:
    ...
    tree()
    {
        std::cout << "树类 删除成功!" << std::endl;
    }
    void showName()
    {
        std::cout << "此树是:" << _name << std::endl;
    }
...
{
    tree w("scp_038", "SCP-038有能力克隆任何接触到它的树皮的物体。接触到树皮的物体几乎立即长出并且成熟，这一过程仅仅需要数分钟。此前的记录表明存在长出的没每件物体重量不能超过90.9kg(200lb)的重量限制。通过SCP-038进行人和动物的克隆是不被推荐的，因为他们非常快速的生长到了母体的原有年龄。这些克隆体的大部份能生存，平均来说，2个星期。对那些死去克隆体的解剖分析表明，它们在死前已经开始发酵。");
}
//     // scanf("%d", &N);
