...
{
  public:
    pig(std::string _name, std::string _size, std::string _colour) : _name(_name), _size(_size), _colour(_colour)
    {
    ...
    void show()
    {
        std::cout << _name << "\t" << _size << "\t" << _colour << std::endl;
    }

  private:
    std::string _name, _size, _colour;
};
int main()
{
    pig a("pig", "大", "pink");
    ...
    std::string *a = new std::string("111");
    ...
    *ptr_year = a;
    ...
    /*int num[5];
    int *nums = new int[5];
    delete[] nums; //释放数组delete[]name
    int arrays[]{20, 60, 40, 30, 10};
    int *p_arrays = arrays;
    for (int i = 0; i < 5; i++)
    {
        std::cout << *p_arrays + i << std::endl;
    }
    for (int i = 0; i < 5; i++)
    {
        std::cout << *p_arrays + i << std::endl;
