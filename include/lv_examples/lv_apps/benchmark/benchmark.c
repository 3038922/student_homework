 ...
 *      DEFINES
 *********************/
#define TEST_CYCLE_NUM                                                         \
  10 /*How many times run the test (will calculate the average)*/
 ...
 *  STATIC PROTOTYPES
 **********************/
static void refr_monitor(lv_disp_drv_t *disp_drv, uint32_t time_ms,
                         uint32_t px_num);
...
void benchmark_create(void) {
  ...
  lv_obj_set_protect(
      wp, LV_PROTECT_PARENT); /*Don't let to move the wallpaper by the layout */
...
uint32_t benchmark_get_refr_time(void) {
  if (benchmark_is_ready())
    return time_sum / TEST_CYCLE_NUM;
  else
    return 0;
}
...
static void recolor_btn_event_cb(lv_obj_t *btn, lv_event_t event) {
  ...
  if (lv_btn_get_state(btn) == LV_BTN_STATE_TGL_REL)
    style_wp.image.intense = IMG_RECOLOR;
  else
    style_wp.image.intense = LV_OPA_TRANSP;
